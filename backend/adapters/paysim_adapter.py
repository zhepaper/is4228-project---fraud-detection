...
from backend.core.feature_roles import FEATURE_ROLES, get_common_features, get_all_features
from backend.core.preprocessing import process_monetary
...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Steps:
      0. Cast numeric columns from strings (handles API raw inputs)
    ...
    num_cols = [c for c in ['amount', 'oldbalanceOrg', 'newbalanceOrig', 'oldbalanceDest', 'newbalanceDest', 'step'] if c in df.columns]
