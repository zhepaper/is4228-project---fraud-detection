...
from fraud_pipeline.core.decision import make_decision, get_risk_factors
from fraud_pipeline.adapters import paysim_adapter, baf_adapter, ieee_adapter, finbank_adapter
...
def load_models():
    ...
    # --- PaySim ---
    MODELS["paysim"] = {
        "model"     : joblib.load(os.path.join(MODELS_DIR, "paysim_xgb.pkl")),
        "scaler"    : joblib.load(os.path.join(MODELS_DIR, "paysim_scaler.pkl")),
        "artifacts" : joblib.load(os.path.join(MODELS_DIR, "paysim_artifacts.pkl")),
    }

    # --- BAF ---
    baf_folds = [
        joblib.load(os.path.join(MODELS_DIR, f"baf_lgbm_fold{i}.pkl"))
        for i in range(1, 6)
    ]
    MODELS["baf"] = {
        "models"    : baf_folds,
        "encoder"   : joblib.load(os.path.join(MODELS_DIR, "baf_encoder.pkl")),
        "artifacts" : joblib.load(os.path.join(MODELS_DIR, "baf_artifacts.pkl")),
    }

    # --- IEEE ---
    ieee_folds = [
        joblib.load(os.path.join(MODELS_DIR, f"ieee_lgbm_fold{i}.pkl"))
        for i in range(1, 6)
    ]
    MODELS["ieee"] = {
        "models"         : ieee_folds,
        "label_encoders" : joblib.load(os.path.join(MODELS_DIR, "ieee_label_encoders.pkl")),
        "drop_cols"      : joblib.load(os.path.join(MODELS_DIR, "ieee_drop_cols.pkl")),
        "artifacts"      : joblib.load(os.path.join(MODELS_DIR, "ieee_artifacts.pkl")),
    }
...
def _predict_paysim(features: dict) -> PredictResponse:
    ...
    # Risk factors from feature values (SHAP not available for XGBoost without extra setup)
